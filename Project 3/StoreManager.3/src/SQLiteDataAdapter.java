...
*/
    ...
    public void saveNewAccount(NewAccountModel account) {
        try {
            ...
            if (loadNewAccount(account.userName) == null) {           // this is a new order!
                stmt.execute("INSERT INTO User(UserName, Password, DisplayName, UserRole) VALUES ("
                        + '\'' + account.userName + '\'' + ","
                        + '\'' + account.password + '\'' + ","
                        + '\'' + account.displayName + '\'' + ","
                        + '\'' + "Customer" + '\'' + ")"
                        // I do not believe we have to add a userRole here since it's default.
                );
            }
            else {
    ...
    public NewAccountModel loadNewAccount(String userName) {
        NewAccountModel account = null;
        try {
            Statement stmt = conn.createStatement();
            ResultSet rs = stmt.executeQuery("SELECT * FROM User WHERE UserName = '" + userName +"'");
            if (rs.next()) {
                account = new NewAccountModel();
                account.userName = rs.getString(1);
                account.password = rs.getString(2);
                account.displayName = rs.getString(3);
            }
