...
public class RemoteDataAdapter implements DataAccess {
    ...
    @Override
    public NewAccountModel loadNewAccount(String userName) {
        RequestModel req = new RequestModel();
        req.code = RequestModel.LOAD_ORDER_REQUEST;
        req.body = userName;
        ...
        try {
            ...
            if (res.code == ResponseModel.UNKNOWN_REQUEST) {
                ...
                if (res.code == ResponseModel.DATA_NOT_FOUND) {
                    System.out.println("The Server could not find an account with that username!");
                    return null;
                } else {
                    NewAccountModel model = gson.fromJson(res.body, NewAccountModel.class);
                    System.out.println("Receiving a NewAccountModel object");
                    System.out.println("UserName = " + model.userName);
                    System.out.println("Password = " + model.password); //LETS MUTE THIS IS WE CAN
    ...
    @Override
    public void saveNewAccount(NewAccountModel account) {
        RequestModel req = new RequestModel();
        req.code = RequestModel.SAVE_ACCOUNT_REQUEST;
        req.body = gson.toJson(account);
        ...
        try {
            ...
            if (res.code == ResponseModel.UNKNOWN_REQUEST) {
                ...
                if (res.code == ResponseModel.DATA_NOT_FOUND) {
                    System.out.println("The Server could not save the NewAccount!");
                } else {
                    NewAccountModel model = gson.fromJson(res.body, NewAccountModel.class);
                    System.out.println("Receiving a NewAccountModel object");
                    System.out.println("User Name = " + model.userName);
                    System.out.println("Password = " + model.password); //SECURITY CONCERN (CAN THIS BE MUTED?)
                    System.out.println("displayName = " + model.displayName);
                }
            }
        } catch (Exception ex) {
            ex.printStackTrace();
